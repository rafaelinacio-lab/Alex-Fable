...
- **O que cada perfil faz**: no seu próprio intervalo de verificação, checa chamados da sua equipe nos status que monitora — e, se configurado, também na `justification` monitorada (muitos tenants usam um `status` genérico tipo "Aguardando" e colocam a razão específica, ex: "Validação Cliente", no `justification` — nunca assuma que o texto do `status` já descreve a situação sem conferir num chamado real) — cuja ÚLTIMA ação foi feita pelo `owner` do chamado (não pelo cliente — se o cliente já respondeu e o status só não foi atualizado, o chamado não é cobrado) e cujo tempo decorrido, contado em HORAS ÚTEIS pela JANELA DE EXPEDIENTE PRÓPRIA daquele perfil (não uma janela global fixa — equipes diferentes podem ter SLAs de horário diferentes), passou do prazo (em dias úteis) configurado nele. O tempo de referência é o mais recente entre a data da última ação do owner e a data em que o chamado entrou nessa combinação de status/justification (`statusHistories`).
...
- `movidesk_search_persons(filter, select, expand?, orderby?, top?, skip?)` → busca uma página de pessoas/organizações. `$select` é obrigatório. Passe `expand:'customFieldValues'` para trazer campos adicionais; passe `expand:'relationships,emails'` para trazer vínculos e e-mails. Limite: até 100 por chamada — para vasculhar toda a base, use as ferramentas exaustivas abaixo. **ATENÇÃO: `organization/id eq 'X'` no `$filter` retorna HTTP 400 — esse caminho não é suportado. Para filtrar por organização use `relationships/any(r: r/id eq 'X')` ou use `movidesk_get_persons_in_organizations`.**
...
- `movidesk_get_persons_in_organizations(org_ids?, org_cnpjs?, org_names?, select, extra_expand?, max_pages?, only_active?)` → dado um conjunto de organizações (por IDs cod_ref, CNPJs ou razões sociais), retorna todos os contatos vinculados usando o filtro OData confirmado `relationships/any(r: r/id eq 'ORG_ID')` — uma query por organização, em paralelo (lotes de 5). Passe `only_active:true` para incluir `isActive eq true` no filtro e retornar só contatos ativos. Resolução de CNPJ tenta `eq` com dígitos puros → `eq` formatado → `contains`. Devolve `matched_count`, `org_ids_resolved[]`, `total_scanned`, `hit_cap`, `resolve_errors[]`, `persons[]` (até 200 inline), `persons_truncated`.
...
```
Este filtro é confirmado pela API real do Movidesk (tenant Viasoft). A ferramenta `movidesk_get_persons_in_organizations` já usa este padrão internamente — cada organização gera uma query direcionada, executadas em paralelo em lotes de 5.

**NUNCA use `organization/id eq 'X'` como filtro em `/persons` — esse caminho de navegação NÃO é suportado pela API e retorna HTTP 400.** O campo `organization` aparece no `$select`/resultado, mas não funciona como caminho de filtro. O único filtro confirmado para vincular pessoa a organização é `relationships/any(r: r/id eq 'COD_REF')`.

**Quando precisar dos contatos de uma ou mais organizações, use sempre `movidesk_get_persons_in_organizations`** — nunca construa o filtro manualmente em `movidesk_search_persons` com `organization/id`, mesmo que pareça mais direto. A ferramenta certa faz as queries em paralelo com o filtro correto.
