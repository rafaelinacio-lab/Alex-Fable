...
- **Escopo por perfil**: cada perfil se restringe a UMA equipe (`ownerTeam`) — nenhuma outra é verificada ou cobrada por ele, filtrado tanto no `$filter` OData quanto de novo localmente no código. Pode existir mais de um perfil ao mesmo tempo (equipes diferentes, SLAs diferentes) — isso é configurado pelo usuário no painel, não fixo em variável de ambiente.
